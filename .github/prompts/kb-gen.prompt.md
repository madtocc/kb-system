---
description: 'Generate a focused COPILOT.md working set from the KB System for a specific story'
agent: 'agent'
...
3. If neither exists, but `generate-context-md.mjs` exists in one of those locations, use that with `COPILOT.md` as the output file.
...
2. Run the generator so it writes `COPILOT.md`:

```bash
node .kb/scripts/generate-copilot-md.mjs <story-path-or-id>
```

3. Read `COPILOT.md` before taking implementation actions.
4. Verify that:
...
6. If the user asked to implement the story, continue the task using `COPILOT.md` as the scoped context. Do not widen scope unless the story itself requires it.
...
- Keep `COPILOT.md` disposable. Regenerate it instead of editing it manually.
